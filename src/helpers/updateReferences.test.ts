...
// List of unique references that are included in input story json, with updated `label` field
// including the reference index as a numbering value
const REFERENCES_DATA: ReferenceProps[] = [
  {
...
describe("createReferencesData helper", () => {
  ...
  test("returns array of unique `ReferenceProps` items including new label as a number", () => {
    // Json story contains reference data
    const referencesStory: ISbStoryData = {
      ...JSON.parse(story).story,
    };
...
describe("updateStoryReferences helper", () => {
  test("returns unmodified story if input story contains no content", () => {
    const emptyContentStory: ISbStoryData = {
      ...JSON.parse(story).story,
      content: {},
    };
    ...
    expect(returnedStory).toEqual(emptyContentStory);
  });

  test("returns unmodified story if input story contains no references as part of the content", () => {
    const emptyReferencesStory: ISbStoryData = {
      ...JSON.parse(story).story,
      // Just some placeholder content that doesn't include references
      content: {
    ...
    const returnedStory = updateStoryReferences(
      emptyReferencesStory,
      REFERENCES_DATA,
    );
    expect(returnedStory).toEqual(emptyReferencesStory);
  });

  test("returns modified story if input story contains matching references as part of the content", () => {
    // Json story contains reference data
    const referencesStory: ISbStoryData = {
      ...JSON.parse(story).story,
    };
